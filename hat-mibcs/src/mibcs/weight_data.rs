...
impl WeightData {
    pub fn parse(address : &str, ads : &[bluetooth::advertising_data::AdvertisingData]) -> Option<WeightData> {
        if ads.len() != 3 {
            return None
        }
        ...
        Some(WeightData { address : address.to_string(), weight, impedance, statusbits0, statusbits1, got_impedance, got_weight, weight_stabilized, impedance_stabilized })
    }
